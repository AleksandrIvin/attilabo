...
IR_process()
{
    ...
    case IR_EVENT_INIT:
        ...
        if (value == LOW)
        {
            ...
            if (counter > (IR_NEC_CODE_PULSE_BURST_9_0_MS - 30) && counter < (IR_NEC_CODE_PULSE_BURST_9_0_MS + 30))
            {
                ...
                IR_timeout = IR_EVENT_IDLE_TIMEOUT;
            }
            else
            {
...
ISR(TIM0_COMPA_vect)
{
    ...
    is set to 38.222kHz. */
    if (IR_counter++ > IR_COUNTER_LIMIT)
        IR_event = IR_EVENT_IDLE;
